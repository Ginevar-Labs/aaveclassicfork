...
import { SingleDeposit } from './single';
   ...
   return (
     <>
      ...
      <Container>
           ...
           {tokenList?.map((token, index) => {
             ...
             <Table.Row key={index} >
               ...
               <Table.Cell css={{display:"flex"}}>
                  ...
                  <Button auto light color={"secondary"} css={{marginTop:"4px"}} onPress={() => {setActiveToken(index);setMarketMode(1)}}>
                    {icons.activity}
                  </Button>
               </Table.Cell>
       ...
       : <SingleDeposit onChange={(id) => setMarketMode(id)}/>}
       </>
   );
};
